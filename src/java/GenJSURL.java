...
public class GenJSURL {
    ...
    private static final Log LOG = LogFactory.getLog(GenJSURL.class);
    ...
    private String gcreateURLInternal(Map parameterValues, String encoding, boolean includeContextPath) {
       ...
       // StringBuffer uri = new StringBuffer(contextPath);//YIGUANG Changed for javascript
  ...
  for (int i = 0; i < tokens.length; i++) {
        ...
        return  "javascript:runRemote(\""+uri.toString()+"\");";
    }
...
/*	if(parameterValues.containsKey(OFFSET)) {
	...
	parameterValues.remove(OFFSET);
        ...
        for (Iterator i = parameterValues.keySet().iterator(); i.hasNext();) {
            ...
            if(!usedParams.contains(name)) {
                if(!addedParams) {
                    uri.append(QUESTION_MARK); //YIGUANG CHANGED IT from QUESTION_MARK
                    addedParams = true;
                }
                else {
                ...
                if(value != null && value instanceof Collection) {
                    ...
                    for (Iterator j = multiValues.iterator(); j.hasNext();) {
                        ...
                        if(j.hasNext()) {
                            uri.append(AMPERSAND); //YIGUANG CHANGED IT from AMPERSAND
	...
	if(ofset!=null){
                    uri.append(AMPERSAND).append(ofset);    //YIGUANG CHANGED IT from AMPERSAND
	}
    }
